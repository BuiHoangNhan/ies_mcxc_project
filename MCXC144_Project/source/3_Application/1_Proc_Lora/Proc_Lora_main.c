...
{
    LOG("Node receives SEND_CFG command from Gateway\r\n");
    UserData_getPointer()->SystemData.Provision_state = PROVISION_SUCCESS;
    LOG("Provision state: SUCCESS\r\n");
    UserData_getPointer()->Node_userConfig.current_thres = data.current_thres / 100;
    LOGF("Current threshold: %d\r\n", UserData_getPointer()->Node_userConfig.current_thres);
    UserData_getPointer()->Node_userConfig.vol_thres = data.vol_thres / 100;
    LOGF("Voltage threshold: %d\r\n", UserData_getPointer()->Node_userConfig.vol_thres);
    UserData_getPointer()->Node_userConfig.power_thres = data.power_thres / 100;
    LOGF("Power threshold: %d\r\n", UserData_getPointer()->Node_userConfig.power_thres);
    UserData_save();
    Lora_send_Ack(0, data.cmd_id); // Reply ACK to Gateway
}
#endif
void Lora_set_toProvision(void)
{
...
{
    LOGF("Node Received REQ_ID:\r\n");
    LOGF("  lora_id: 0x%04X\r\n", lora_id);
    LOGF("  CSMA_time: %d\r\n", data.CSMA_time);
    LOGF("  reset_rf_time: %d\r\n", data.reset_rf_time);
    LOGF("  max_tx_power: %d\r\n", data.max_tx_power);
    LOGF("  router_score: %d\r\n", data.router_score);
    LOGF("  data_period: %d\r\n", data.data_period);
    LOGF("  propagation_radius: %d\r\n", data.propagation_radius);
    LOGF("  PainID: 0x%02X\r\n", Lora_data_revc.PainID);
    LOGF("  Assigned ChannelID: %d\r\n", UserData_getPointer()->SystemData.ChannelID);
    ...
    {
        LOG("  Provision status: NONE → changing to PENDING\r\n");
