...
static int OTA_write_magic_ota(void);
#define APP_IMAGE (0)
...
static uint8_t OTA_progbuf[BUFFER_1KB]; //buffer OTA progress
static hashctx_t OTA_sha256_xmodem_ctx;
static partition_t OTA_prt;
static strtOTA_State strOTAstate ={0};
static timer_t OTA_timer ;
...
int OTA_done(void)
{
    ...
    {
        process_received_data(strOTAstate.OTA_offset, sizeof(OTA_progbuf));
        strOTAstate.OTA_offset += sizeof(OTA_progbuf);
        strOTAstate.OTA_buf_offset = 0;
        memset(OTA_progbuf, 0xff, sizeof(OTA_progbuf));
    }
   uint32_t cur_offset = strOTAstate.OTA_offset;
   uint32_t remain_size = BOOT_FLASH_CAND_APP - BOOT_FLASH_ACT_APP;
   for(uint16_t i = 0; i < (remain_size- cur_offset)/sizeof(OTA_progbuf); i++){
	   if(strOTAstate.OTA_offset == remain_size -sizeof(OTA_progbuf) )
	   {
		   OTA_write_magic_ota();
	   }
	   process_received_data(strOTAstate.OTA_offset, sizeof(OTA_progbuf));
    ...
    strOTAstate.OTA_offset = remain_size;
    ota_log_info("\nReceived %u bytes\n", strOTAstate.OTA_offset);
    ...
    if (ret == 0 && bl_verify_image(BOOT_FLASH_CAND_APP, remain_size) == 1){
    	HAL_ResetMCU();
    }
    else{
        ret = 2;
...
static int OTA_write_magic_ota(void){
    uint32_t sizeof_boot_img_magic = 16;
    uint8_t image_magic[] = {
        0x77, 0xc2, 0x95, 0xf3,
        0x60, 0xd2, 0xef, 0x7f,
        0x35, 0x52, 0x50, 0x0f,
    ...
    memcpy(&OTA_progbuf[sizeof(OTA_progbuf)-sizeof_boot_img_magic], image_magic, sizeof(image_magic));
}
void OTA_Timeout(void)
{
    if(OTA_getState() != eOTA_Idle)
    {
        if(Timer_Timeout(&OTA_timer))
        {
        	OTA_setState(eOTA_Idle);
        }
    }
}
void OTA_refreshTimer(void)
{
    Timer_Create(&OTA_timer, OTA_TIMEOUT);
}
/* End of ota.c */
